import {asyncTaskActions, slice} from '../tasks-reducer';
...
const {addTodolist, removeTodolist, fetchTodoLists} = asyncActions;
const {removeTask, addTask, updateTask, fetchTasks} = asyncTaskActions;
...
test('correct task should be added to correct array', () => {
  ...
  const endState = tasksReducer(startState, addTask.fulfilled(newTask, 'requestId', {title: newTask.title, todolistId: newTask.todoListId}))
  expect(endState['todolistId1'].length).toBe(3)
...
test('new array should be added when new todolist is added', () => {
  ...
  const endState = tasksReducer(startState, addTodolist.fulfilled({todolist: newTodolist}, 'requestId', 'New Todolist'))
...
test('property with todolistId should be deleted', () => {
  const endState = tasksReducer(startState, removeTodolist.fulfilled({id: 'todolistId2'}, 'requestId',  'todolistId2'))
  const keys = Object.keys(endState);
...
test('empty arrays should be added when we set todolists', () => {
  ...
  const action = fetchTodoLists.fulfilled(getTodolists, 'requestId', undefined)
